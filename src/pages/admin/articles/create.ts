...
export const POST: import('astro').APIRoute = async ({ request, locals, redirect }) => {
  try {
    ...
    const result = await db
      .prepare(
        `INSERT INTO articles (title, slug, excerpt, article_type, status, subject_id, topic_id, author_id, reading_minutes)
         VALUES (?, ?, ?, ?, 'draft', ?, ?, ?, 1)`,
      )
      .bind(title, slug, excerpt, articleType, subjectId, topicId, user.id)
      .run();
